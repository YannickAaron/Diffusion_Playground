...
def plot_svc_decision_function(model, ax=None, plot_support=True):
    ...
    ax.contour(X, Y, P, colors="w", levels=[-1, 0, 1], alpha=0.9, linestyles=["--", "-", "--"])

    # plot support vectors
    if plot_support:
        ax.scatter(model.support_vectors_[:, 0], model.support_vectors_[:, 1], s=300, linewidth=2, edgecolor="w", facecolors="none")
    ax.set_xlim(xlim)
    ax.set_ylim(ylim)
...
def fit_multivariate_gaussian(X_s):
    ...
    plt.scatter(X_s[:, 0], X_s[:, 1], c="b", s=60, edgecolor="w", zorder=2.5)
    plt.grid(True)
    return labels
